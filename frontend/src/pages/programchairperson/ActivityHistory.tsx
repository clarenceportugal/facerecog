...
const ActivityHistory: React.FC = () => {
  ...
  const fetchSystemActivities = async () => {
    try {
      ...
      const userRole = localStorage.getItem("role") || "programchairperson";
      ...
      if (response.data.success && Array.isArray(response.data.activities)) {
        ...
        console.log(`[ActivityHistory] ✅ Loaded ${activities.length} system activities from backend`);
        console.log(`[ActivityHistory] Breakdown:`, {
        ...
        console.error("[ActivityHistory] Invalid response format:", response.data);
        setActivities([]);
      }
    } catch (error: any) {
      console.error("[ActivityHistory] ❌ Error fetching system activities:", error);
      console.error("[ActivityHistory] Error details:", error.response?.data || error.message);
      setActivities([]);
    } finally {
      setLoading(false);
    }
